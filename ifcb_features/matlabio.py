...
class Matlab:
    ...
    def run(self, command):
        ...
        try:
            script = (
                "%s; try, disp('%s'), %s, catch err, disp(err.message), disp(err.stack), disp(err.identifier), exit(1), end, exit(0)"
                % (pathcmds, self.output_separator, command)
            )
            cmd = [self.exec_path, "-nodisplay", "-r", script]
            p = subprocess.Popen(
                    ...
                    while True:
                        ...
                        if seen_separator:
                            try:
                                ...
                                self.log_callback(
                                    "Output callback raised exception; killing Matlab process %d"
                                    % p.pid
                                )
                                make_dead(p)
                                raise
            ...
            if p.returncode is not None and p.returncode != 0 and self.fail_fast:
                ...
                raise RuntimeError(
                    "Nonzero return code (%d) from Matlab process %d"
                    % (p.pid, p.returncode)
                )
